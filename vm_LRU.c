...
typedef struct{
	...
	int capacity;//容量
	struct hash* table;
	struct node* recent;//表头
	struct node* unrecent;//表尾
} LRUCache;
...
int main(int argc,char* argv[])
{
	...
	while(fscanf(rd,"%d",&fp_temp) != EOF)
	{
		...
		int w=0;
		if(Get_LRUCache(page_TLB,page_num) == -1){
			printf("命中失败");
		}
		else{
			page_num = Get_LRUCache(page_TLB,page_num);
			TLBHitTimes++;
		}
			...
			if(count/128){
				...
				for(w=0;w<PAGESIZE;w++)
					if(page_table[w] == (count % 128))
						page_table[w] = -1;
				Put_LRUCache(page_TLB,count % 128,page_num);
				update = 0;//已经更新,不用置换
				}
		...
		if(update){
			Put_LRUCache(page_TLB,va,page_num);
		}
